...
class Global {
  ...
  mfnApiValidate(e,fnCallBack){
    Future<dynamic> futureValidate;
    if(e["TYPE"] == "SCH"){
      futureValidate = ApiCall().apiLookupValidateSch(e["TABLE"]??"", e["KEY"]??"", e["CONTROLLER"].text,e["COMPANY"].toString());
    }else{
      futureValidate = ApiCall().apiLookupValidate(e["TABLE"]??"", e["KEY"]??"", e["CONTROLLER"].text);
    }

    futureValidate.then((value){
      fnCallBack(value,e);
    });
  }
